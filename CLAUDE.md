...
skills/
  check-harness/            # 6축 하네스 성숙도 진단 스킬
agents/                     # check-harness가 호출하는 전용 서브에이전트
```

## Development Guidelines

- `skills/check-harness/SKILL.md`가 메인 실행 계약이다.
- `skills/check-harness/references/checklist.md`는 6축/24항목 판정 기준의 원본이다.
- `agents/*.md`는 읽기 전용 분석 에이전트로 유지한다.
